...
const PermissionEditSetting = () => {
  const { id, account } = useParams();

  const dispatch = useDispatch();

  const { allMerchants, proxyState, merchantState, editProxy } = useSelector(
  ...
  }, [dispatch, merchantState]);

  useEffect(() => {
    dispatch(groupListService.getEditProxy(id));
  }, [dispatch, id]);
  ...
  useEffect(() => {
    ...
    if (Object.keys(editProxy).length !== 0) {
      ...
      editProxy.members.map((item, key) => {
        tempTargetKeys.push(item._id);
      });
      setTargetKeys(tempTargetKeys);
    }
  }, [allMerchants, editProxy]);
  ...
  const handleClickSave = () => {
    dispatch(
      groupListService.changeMembersInProxy({ id: id, members: targetKeys })
    );
  };
          ...
          <div className="relative overflow-hidden">
              ...
              <div className="outline-none w-full border-b border-[#d9d9d9] transition duration-300 flex flex-row justify-between">
                  ...
                  <div className="overflow-hidden -mb-[1px]">
                    <div className="overflow-hidden whitespace-nowrap">
                      <div className="pl-0 transition-transform duration-300 relative m-0 inline-block transform translate-x-0 translate-y-0 scale-100 list-none">
                        <div className="block bg-primary visible transform translate-x-0 translate-y-0 scale-100 w-[233px] transition-all duration-300 absolute left-0 bottom-[1px] h-[2px] origin-top-left"></div>
                        <div className="mr-0 inline-block h-full relative pt-2 pb-2 pr-5 pl-5 transition-colors duration-300 cursor-pointer no-underline">
                          <font className="text-primary">
                            {t(
                              "userRightsManagement.permissionSettings.createNewPermissionGroup"
                            )}
                          </font>
                        </div>
                      </div>
                ...
                <div className="">
                  <div className="leading-8">
                    <button
                      className="text-[#fff] mr-1 inline-block 
                      mb-0 
                      font-medium 
                      text-center 
                      pointer-events-auto 
                    ...
                    <button
                      className="text-[#fff] inline-block 
                      mb-0 
                      font-medium 
                      text-center 
                      pointer-events-auto 
                    ...
                    >
                      <font>
                        {t(
                          "userRightsManagement.permissionSettings.returnPermissionGroupList"
                        )}
                      </font>
                    </button>
                  </div>
            ...
            <div className="flex">
              <div className="w-1/2 pr-10">
                <div className="flex mt-5 p-2">
                  <div className="w-1/2 pr-5">
                    <label
                      for="mySelect"
                      className="block text-[14px] text-[#rgba(0,0,0,0.65)]"
                    >
                      {t(
                        "userRightsManagement.permissionSettings.selectGroupType"
                      )}
                    </label>
                    <select
                      ...
                      id="mySelect"
                      name="mySelect"
                      className="min-w-[180px] text-xs px-2 py-[3px] text-base border border-grayCustom focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    >
                      <option>
                        {t("userRightsManagement.permissionSettings.merchant")}
                      </option>
                      <option>
                        {t(
                          "userRightsManagement.permissionSettings.subMerchant"
                        )}
                      </option>
                      <option selected>
                        {t(
                          "userRightsManagement.permissionSettings.superAdministrator"
                        )}
                      </option>
                      <option>
                        {t("userRightsManagement.permissionSettings.acting")}
                      </option>
                      <option>
                        {t(
                          "userRightsManagement.permissionSettings.squadLeader"
                        )}
                      </option>
                    </select>
                  </div>
                  <div className="w-1/2 pl-5">
                    ...
                    >
                      {t(
                        "userRightsManagement.permissionSettings.selectGroupName"
                      )}
                    </label>
                    <input
                      placeholder=""
                      className="w-full h-7 text-xs px-2 pl-3 pr-[7px] py-o text-base border border-grayCustom focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"
                    />
                  </div>
                </div>
                <div className="flex mt-5 p-2">
                          ...
                          return (
                            ...
                            >
                              <label>
                                ...
                                />
                                {t(
                                  `userRightsManagement.permissionSettings.${item}`
                                )}
                              </label>
                            </li>
                          );
              ...
              <div className="mt-7 w-1/2 pl-10">
                <label>
                  {t("userRightsManagement.permissionSettings.pleaseSetPeople")}
                  :
                </label>
                ...
                <div className="mt-10 text-center">
                  <button
                    ...
                    className="mr-3 py-1 px-3 rounded text-white bg-primary hover:bg-primary-dark"
                  >
                    {t(
                      "userRightsManagement.permissionSettings.confirmAndSave"
                    )}
                  </button>
                  <button className="py-1 px-3 rounded text-primary bg-grey-light">
                    {t("userRightsManagement.permissionSettings.reset")}
                  </button>
                </div>
              </div>
            </div>
...
export default PermissionEditSetting;
